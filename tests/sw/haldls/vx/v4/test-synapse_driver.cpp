...
TEST(SynapseDriverConfig, General)
{
	...
	value = !config.get_enable_hagen_dac();
	config.set_enable_hagen_dac(value);
	ASSERT_EQ(config.get_enable_hagen_dac(), value);
	...
	auto hagen_dac = SynapseDriverConfig::HagenDACOffset(config.get_hagen_dac_offset() + 1);
	config.set_hagen_dac_offset(hagen_dac);
	ASSERT_EQ(config.get_hagen_dac_offset(), hagen_dac);
...
TEST(SynapseDriverConfig, EncodeDecode)
{
	...
	config.set_hagen_dac_offset(SynapseDriverConfig::HagenDACOffset(21));
	...
	std::array<OmnibusChipOverJTAGAddress, SynapseDriverConfig::config_size_in_words>
	    ref_addresses = {
	        OmnibusChipOverJTAGAddress{0x118400 + 42 * 4 + 0},
...
TEST(SynapseDriverConfig, CerealizeCoverage)
{
	...
	c1.set_enable_hagen_dac(true);
	c1.set_recovery(SynapseDriverConfig::Recovery(5));
	c1.set_hagen_dac_offset(SynapseDriverConfig::HagenDACOffset(63));

...
{
	if constexpr (std::is_same_v<Container, lola::vx::v4::ExternalPPUMemoryBlock>) {
		return 1;
	} else if constexpr (std::is_same_v<Container, haldls::vx::v4::PPUMemoryBlock>) {
		return 2;
	} else if constexpr (std::is_same_v<Container, lola::vx::v4::LogicalNeuron>) {
		return 1;
	} else {
...
test_write()
{
	...
	stadls::vx::v4::PlaybackProgramBuilder builder;
	hate::Timer timer;
	for (size_t i = 0; i < num_containers; ++i) {

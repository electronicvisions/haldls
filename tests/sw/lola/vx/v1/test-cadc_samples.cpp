...
TEST(CADCSamples, EncodeDecode)
{
	...
	ref_data[12] = fisch::vx::word_access_type::Omnibus(0xf);
	ref_data[14 + SynapseQuadColumnOnDLS::size] = fisch::vx::word_access_type::Omnibus(0x10);
	...
	ASSERT_EQ(config.causal[SynramOnDLS::top][SynapseOnSynapseRow(43)], CADCSamples::Value(0xf0));
	ASSERT_EQ(config.acausal[SynramOnDLS::top][SynapseOnSynapseRow(51)], CADCSamples::Value(0x8));
}

...
const PPUMemoryWordOnDLS stack_begin(3071);
const PPUMemoryWordOnDLS mailbox_begin(3072);
const PPUMemoryWordOnDLS mailbox_end(4095);
const PPUMemoryWordOnDLS return_code(3071);

const std::pair mailbox_coord(mailbox_begin, mailbox_end);
...
int main(int argc, char* argv[])
{
	...
	auto ppu_memory_program_coord = std::pair(PPUMemoryWordOnDLS(0), PPUMemoryWordOnDLS(ppu_memory_program.size() - 1));
	auto ppu_memory_stack_heap_coord =
	    std::pair(PPUMemoryWordOnDLS(ppu_memory_program.size() - 1), stack_begin);
	...
	if (vm.count("board-id")) {
		ctrl = ExperimentControl(test_board);
	}
	ctrl.run_experiment(board, chip, program);

	// get results

	auto status_register = program.get(ppu_status_ticket);
	...
	auto ppu_memory_program_after = program.get(ppu_memory_program_ticket);
	...
	uint32_t exit_code = program.get(ppu_memory_return_ticket).get();
	...
	if (vm.count("dump-stack-heap")) {
		auto ppu_memory_stack_heap_after = program.get(ppu_memory_stack_heap_ticket);
	...
	if (vm.count("dump-memory")) {
		auto ppu_memory_after = program.get(ppu_memory_ticket);
	...
	if (vm.count("print-mailbox")) {
		auto ppu_memory_mailbox_after = program.get(ppu_memory_mailbox_ticket);
		std::stringstream mailbox;
		mailbox << "Printing mailbox." << std::endl;
		mailbox << helpers::print_PPUMemoryBlock_as_string(ppu_memory_mailbox_after) << std::endl;

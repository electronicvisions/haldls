...
Chip const Chip::default_neuron_bypass = []() {
	...
	for (auto const block : iter_all<SynapseDriverBlockOnDLS>()) {
		...
		value.fill(haldls::vx::v2::CommonPADIBusConfig::DacenPulseExtension(
		    haldls::vx::v2::CommonPADIBusConfig::DacenPulseExtension::max));
...
EXPLICIT_INSTANTIATE_CEREAL_SERIALIZE_FREE(lola::vx::v2::Chip)

...
public:
	...
	void configure_static(haldls::v2::Board const& board, haldls::v2::Chip const& chip) HALDLS_VISIBLE;

	/// \brief transfers the program and sets the program size and address
	///        registers
	void transfer(haldls::v2::PlaybackProgram const& playback_program) HALDLS_VISIBLE;
	/// \brief toggle the execute flag and wait until turned off again
	void execute() HALDLS_VISIBLE;
	void fetch(haldls::v2::PlaybackProgram& playback_program) HALDLS_VISIBLE;

	/// \brief this just wraps the sequence transfer-execute-fetch
	void run(haldls::v2::PlaybackProgram& playback_program) HALDLS_VISIBLE;

...
template <>
struct BackendContainerTrait<lola::vx::ExternalPPUDRAMMemoryBlock>
    : public BackendContainerBase<
...
template <>
struct VisitPreorderImpl<lola::vx::ExternalPPUDRAMMemoryBlock>
{
	template <typename ContainerT, typename VisitorT>
	static void call(
	    hate::Empty<ContainerT> const& config,
	    lola::vx::ExternalPPUDRAMMemoryBlock::coordinate_type const& coord,
	    VisitorT&& visitor)
	{
		using namespace halco::hicann_dls::vx;
		...
		hate::Empty<ExternalPPUDRAMMemoryQuad> quad_config;
		for (size_t quad = quad_min; quad <= quad_max; ++quad) {
			visit_preorder(quad_config, ExternalPPUDRAMMemoryQuadOnFPGA(quad), visitor);
		}
	}
	    ...
	    VisitorT&& visitor)
	{
		...
		if (coord.toExternalPPUDRAMMemoryBlockSize() != config.size()) {
			std::stringstream ss;
			ss << "container size(" << config.size() << ") and " << coord << " size do not match.";
			throw std::runtime_error(ss.str());
		}
		...
		ExternalPPUDRAMMemoryQuad quad_config;
		for (size_t quad = quad_min; quad <= quad_max; ++quad) {
			quad_config.m_enables.fill(false);
			quad_config.m_quad.fill(ExternalPPUDRAMMemoryByte::Value());
			...
			if constexpr (!std::is_same_v<ContainerT, lola::vx::ExternalPPUDRAMMemoryBlock const>) {
				for (size_t byte = local_byte_min; byte <= local_byte_max; ++byte) {
					EntryOnQuad const entry(byte % sizeof(uint32_t));
					config.m_bytes.at(byte - byte_min).m_value = quad_config.m_quad[entry];
				}
...
template <>
inline lola::vx::ExternalPPUDRAMMemoryBlock coordinate_to_container(
    lola::vx::ExternalPPUDRAMMemoryBlock::coordinate_type const& coord)
{
	return lola::vx::ExternalPPUDRAMMemoryBlock(coord.toExternalPPUDRAMMemoryBlockSize());
}
